-- Fix get_ranked_dishes "vote phantom" bug
--
-- The outer LEFT JOIN to `votes` produces a NULL-row when a dish has no votes.
-- The CASE in the total_votes SUM had `ELSE 1.0`, which fires when v.source IS NULL
-- (neither 'user' nor 'ai_estimated' matches). Every 0-vote dish was therefore
-- being reported as having total_votes=1, which:
--   - Corrupted ranking (search_score depends on total_votes)
--   - Made it impossible for the frontend to distinguish ranked vs unranked dishes
--   - Showed phantom "1 vote" badges on dishes nobody had rated
--
-- Fix: ELSE 1.0 → ELSE 0 in the outer aggregation. The CASE in variant_stats
-- and the avg_rating calculation are untouched — they were already correct
-- (variant_stats sums real votes only; avg_rating uses ELSE 0).
--
-- Codex-confirmed root cause.
...
-- No SQL rollback. To revert, restore the prior get_ranked_dishes from git history at HEAD~1.
