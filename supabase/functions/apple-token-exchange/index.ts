...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (err instanceof AppleApiError) {
      ...
      return json(err.status >= 400 && err.status < 500 ? err.status : 500, {
        ok: false,
        code: 'APPLE_EXCHANGE_FAILED',
        transient: false,
      });
    }
    // Non-AppleApiError thrown — unexpected
    console.error(JSON.stringify({ event: 'apple_token_exchange_unexpected', user_hash: await hashUserId(userId) }));
    return json(500, { ok: false, code: 'APPLE_EXCHANGE_FAILED', transient: false });
  }
