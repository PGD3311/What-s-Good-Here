...
**Winter bug-bash (Dan, 2026-09-21, working on `main` via short-lived fix branches).** Project was idle mid-July → late Sept (summer). Dan is back, walking the app and screenshotting what's off; each finding gets a root-cause fix + unit test + Codex pass + PR. Shipped today: PR #339 (reviews empty-state contradiction, hidden friend reviews, photo Remove unreachable + Replace destructive) and PR #338 (July's cross-device `next` redirect fix, hotline #184 bug half closed). Infra health check passed: Supabase live (not paused), storage + REST + wghapp.com all 200, photo-moderate deployed. Everything in the July "Next up" list below is historical until re-prioritized.
