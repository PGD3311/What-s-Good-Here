...
**Winter bug-bash (Dan, 2026-09-21, `main` via short-lived fix branches).** Project was idle mid-July → late Sept. Dan is back, walking the app and screenshotting what's off; each finding → root-cause fix + unit test + Codex pass + PR. **Shipped today (all live on web; iOS still on the June bundle):** #338 cross-device redirect · #339 reviews empty-state + hidden friend reviews + photo Remove/Replace · #340 hidden-tier photos on profile grid · #341 real photos on ranked rows (+ migration: restaurant RPC photo field, hidden tier in best_photos, Unsplash purge) · #342 one row anatomy (text left, picture slot right) · #343 upload-time dish check (asks, never rejects; photo-moderate → Sonnet 5, redeployed via dashboard) · #344 public profiles empty since April (votes RLS; now reads public_votes + migration) · #346 card rows (superseded same day) · #347 editorial rows — no cards, Fraunces rank numerals, square photo flush to the edge, small-caps Order/Directions (Dan's pick after calling the cards "too AI-y"). Infra: Supabase live, storage + REST + wghapp.com healthy.

**Follow-ups queued from today (not started):** Map FAB hides on scroll (floats over the picture slot) · "wrong dish" report reason + admin reassign-photo action · storage path uses dishId (moved photo can be clobbered by a later upload to the old dish) · `dishesApi.getTrending` reads `votes` cross-user (RLS-broken, dead code — delete) · photo-moderate fail-closed alert in Sentry · icon size in the picture slot (76px) may want to grow · iOS build to carry all of the above.
